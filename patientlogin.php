...
if(isset($_POST['login'])) {
    ...
    if(empty($username)) {
        $error['patient'] = 'Please enter a username';
    }elseif(empty($password)) {
        $error['patient'] = 'Please enter your password';
    }

    if(count($error) == 0) {
        $query = "SELECT * FROM patient WHERE firstname = '$username' AND password = '$password'";
...
<div class="flex">
        <div class="wrapper">
            <h1 style="color: #fff;">Patient Login</h1>
            ...
            <br><br>
            <span><a href="account.php">Click here to Register Yourself</a></span>
        </div>
    </div>
